...
class Canto
{
    ...
    public function getToken()
    {
        ...
        if (count($invalid_credentials) > 0) {
            echo wp_json_encode(array('error' => "Invalid Login Credentials"));
            wp_die();
        }
        ...
        $response_remote_get = wp_safe_remote_get(
            $url,
            array(
                'method' => 'GET',
                'headers' => $headers,
        ...
        if (count($invalid_credentials) > 0) {
            echo wp_json_encode(array('error' => "Invalid AppID"));
            wp_die();
        }
    ...
    {
        wp_register_style(
            $this->_token . '-frontend', esc_url($this->assets_url) . 'css/frontend.css', array(),
            $this->_version
        );
    ...
    {
        wp_register_style(
            $this->_token . '-admin', esc_url($this->assets_url) . 'css/admin.css', array(),
            $this->_version
        );
