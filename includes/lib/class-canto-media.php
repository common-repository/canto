        ...
        if (get_option('fbc_app_expire_token') < time() || ( get_option('fbc_flight_domain') == '' || get_option('fbc_app_token') == '' ) ) {
            ...
            if (get_option('fbc_app_expire_token') < time() ) {
                echo '<form><h3 class="media-title"><span style="font-size:14px;font-family:Helvetica,Arial">' .
                    esc_html__(
                        "<strong>Oops!</strong> Your security token has expired. Please re-authenticate your Canto account. <a href=\"javascript:;\" onclick=\"window.top.location.href='"
                        . get_bloginfo('url') . "/wp-admin/options-general.php?page=canto_settings'\">Plugin Settings</a>", 'canto' 
                    ) . '</span></h3></form>';
            }
            ...
            if (get_option('fbc_flight_domain') == '' || get_option('fbc_app_token') == '' ) {
                echo '<form><h3 class="media-title"><span style="font-size:14px;font-family:Helvetica,Arial">' .
                    esc_html__(
                        "<strong>Oops!</strong> You haven't connected your Canto account yet. <a href=\"javascript:;\" onclick=\"window.top.location.href='"
                             . get_bloginfo('url') . "/wp-admin/options-general.php?page=canto_settings'\">Plugin Settings</a>", 'canto' 
                    ) . '</span></h3></form>';
            }
            ...
            <div class="canto-class">
                <img src="<?php echo esc_url(CANTO_FBC_URL); ?>/assets/loader_white.gif" id="loader">
...
// print_r($_GET);
// die;
        $get_paged = !empty($_GET['paged']) ? sanitize_text_field($_GET['paged']): 0;
